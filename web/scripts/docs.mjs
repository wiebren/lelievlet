...
function manoeuvres() {
  ...
  const leave = stepsOf('Afvaren van langswal');
  const peiling = stepsOf('Dwarspeiling');
  const lager = stepsOf('Aanleggen aan lagerwal');
...
${turnTable(rescue, objectOf('RESCUE_NEEDS'))}
...
${turnTable(peiling, objectOf('SIGHT_NEEDS'))}
...
${turnTable(lager, objectOf('LAGER_NEEDS'))}
...
${turnTable(leaveLager, objectOf('OFF_LAGER_NEEDS'))}
