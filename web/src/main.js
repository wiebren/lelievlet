...
document.getElementById('about-close').addEventListener('click', () => closePanel.get('about')());
document.getElementById('parts-close').addEventListener('click', () => closePanel.get('parts')());
...
new GLTFLoader().load('/models/lelievlet.glb', async (gltf) => {
  ...
  const sails = await dressSails(parts, renderer);
  ...
  quiz = initQuiz({ parts, camera, controls, scene, select, flyTo, startFlight,
                    setHighlights, partVisible, closePanel });        // Oefenen
  setView('iso', false);
...
function buildPartList() {
  ...
  document.getElementById('parts-search').addEventListener('input', filterPartList);
  refreshPartList();
}

/** Filter by name, blind to case and to accents. */
function filterPartList() {
  const query = fold(document.getElementById('parts-search').value.trim());
  let found = 0;
  ...
  document.getElementById('parts-empty').hidden = found > 0;
}
...
function pickTwin(row) {
  ...
  const state = row.twins.map((parts) => {
    const empty = worldBox(parts, twinBox).isEmpty();        // as the parts stand right now, like the flight
    if (!empty) twinBox.getCenter(centre);
    return { parts, there: !empty && parts.some(partVisible),
...
function markRow(part) {
  ...
  if (document.getElementById('parts').hidden) return;
...
function select(list, hit = null) {
  ...
  document.getElementById('info-group').textContent = groups.get(ex.groep)?.title ?? '';
  document.getElementById('info-name').textContent = ex.naam;
  ...
  const handle = handleAt(hit);
  const model = document.getElementById('info-model');
  model.hidden = !handle;
  if (handle) {
    const rest = (ex.handles ?? []).filter((other) => other !== handle);
    const more = document.getElementById('info-more');
    document.getElementById('info-handle').textContent = handle;
...
canvas.addEventListener('pointermove', (e) => {
  ...
  if (hovered) {
    ...
    hoverTip.style.left = `${e.clientX}px`; hoverTip.style.top = `${e.clientY}px`;
  }
...
// ---------------------------------------------------------------- keyboard navigation
// Arrows move the viewpoint, Shift + arrows orbit around the boat, + / - zoom. Applied per frame while held.
const held = new Set();
...
window.addEventListener('keydown', (e) => {
  ...
  if (!NAV_KEYS.has(e.key) || e.target.closest?.('input:not([type=checkbox]), textarea, select, #parts')) return;
  held.add(e.key);
  e.preventDefault();
});
...
const procBar = document.getElementById('procedure');
const procTime = document.getElementById('procedure-time');
const procStep = document.getElementById('procedure-step');
const procName = document.getElementById('procedure-name');
const procTicks = document.getElementById('procedure-ticks');
const procPlay = document.getElementById('procedure-play');
...
document.getElementById('procedure-previous').addEventListener('click', () => modes?.procedureControl.previous());
document.getElementById('procedure-next').addEventListener('click', () => modes?.procedureControl.next());
...
for (const type of ['pointerup', 'pointercancel']) window.addEventListener(type, () => { procDragging = false; });
...
function placeProcedureBar() {
  const panel = document.querySelector('#controls .popover:not([hidden])');
  ...
  document.body.style.setProperty('--procedure-lift', `${lift}px`);
  if (procBar.hidden) return;
  const top = window.innerHeight - procBar.getBoundingClientRect().top;
  document.body.style.setProperty('--procedure-top', `${top + PROC_GAP}px`);
}
...
for (const panel of document.querySelectorAll('#controls .popover')) procRoom.observe(panel);
procRoom.observe(procBar);
window.addEventListener('resize', placeProcedureBar);
...
function stepProcedureBar() {
  ...
  if (!p) { procBar.hidden = true; document.body.classList.remove('procedure-open'); return; }
  ...
  const panel = document.getElementById(p.name === 'Reven' ? 'reef-panel' : 'rig-panel');
  ...
  document.body.classList.toggle('procedure-open', show);
}

if (import.meta.env.DEV) {                       // handy in the console and for automated checks
  window.vlet = { camera, controls, scene, parts, rows, held, keyboardNavigate, select, flyTo,
                  sideOf, pickTwin, get modes() { return modes; }, get regions() { return regions; },
